...
namespace EmoRecognizer
{
    ...
    class FaceTrackerProxy
    {
        ...
        public delegate void FaceDetectedEventHandler(object sender, FaceDetectionEventArgs ea);

        public event FaceDetectedEventHandler OnFaceDetected;
        ...
        private void SetupVisualization(Windows.Foundation.Size framePizelSize, IList<DetectedFace> foundFaces)
        {
            ...
            if (this.currentState == ScenarioState.Streaming && foundFaces != null && actualWidth != 0 && actualHeight != 0)
            {
                ...
                try
                {
                    var x = foundFaces[0].FaceBox.X + foundFaces[0].FaceBox.Width / 2;
                    var y = foundFaces[0].FaceBox.Y + foundFaces[0].FaceBox.Height / 2;
                    OnFaceDetected(this, new FaceDetectionEventArgs() { X = x, Y = y });
                }
                catch { };
            }
        }
